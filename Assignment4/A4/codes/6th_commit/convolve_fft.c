...
void fft(double *x_real, double *x_imag, int size, int inverse) {
    ...
    double dinverse = inverse ? -2.0 : 2.0;
    // FFT

    for (m = 2; m <= size; m <<= 1) {
        double angle = dinverse * PI / m;
        wr = cos(angle);
        wi = sin(angle);
...
void convolutionOverlapAdd(short *x, int M, short *h, int N, short *y) {
    ...
    for (int i = 0; i < L; i++) {
      double yn = fft_x_real[i] + min;
      mean += yn/L;
    }

    double sd = calculateStandardDeviation(fft_x_real, mean, L);
    for (int i = 0; i < L; i++) {
      double yn = (fft_x_real[i]-mean)/(sd);
