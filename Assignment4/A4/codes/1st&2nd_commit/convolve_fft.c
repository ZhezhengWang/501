...
void fft(double *x_real, double *x_imag, int size, int inverse) {
    int i, j, k, n, m, t;
    double temp_real, temp_imag, wr, wi;
    ...
    // FFT
    for (m = 2; m <= size; m <<= 1) {
        double angle = (inverse ? -2.0 * M_PI / m : 2.0 * M_PI / m);
        wr = cos(angle);
        wi = sin(angle);
        ...
        for (i = 0; i < size; i += m) {
            ...
            for (j = 0; j < m / 2; j++) {
                ...
                double u_real = x_real[i + j];
                double u_imag = x_imag[i + j];

                double t_real = w_real * x_real[n] - w_imag * x_imag[n];
                double t_imag = w_real * x_imag[n] + w_imag * x_real[n];
                ...
                double next_w_real = wr * w_real - wi * w_imag;
                double next_w_imag = wr * w_imag + wi * w_real;
...
void convolutionOverlapAdd(short *x, int M, short *h, int N, short *y) {
    ...
    double *fft_x_real = calloc(fftSize, sizeof(double));
    double *fft_x_imag = calloc(fftSize, sizeof(double));
    double *fft_h_real = calloc(fftSize, sizeof(double));
    double *fft_h_imag = calloc(fftSize, sizeof(double));
    double *fft_y_real = calloc(fftSize, sizeof(double));
    double *fft_y_imag = calloc(fftSize, sizeof(double));

    for (int i = 0; i < fftSize; i++) {
        fft_x_real[i] = fft_x_imag[i] = 
        fft_h_real[i] = fft_h_imag[i] = 
        fft_y_real[i] = fft_y_imag[i] = 0.0; 
    }
    ...
    for (int i = 0; i < fftSize; i++) {
        fft_y_real[i] = fft_x_real[i] * fft_h_real[i] - fft_x_imag[i] * fft_h_imag[i];
        fft_y_imag[i] = fft_x_real[i] * fft_h_imag[i] + fft_x_imag[i] * fft_h_real[i];
    }

    // Perform inverse FFT on the result
    fft(fft_y_real, fft_y_imag, fftSize, 1);
    ...
    mean = 0;
    for (int i = 0; i < L; i++) {
      double yn = fft_y_real[i] + min;
      mean += yn/L;
    }
    double sd = calculateStandardDeviation(fft_y_real, mean, L);
    // Extract the real part and scale
    for (int i = 0; i < L; i++) {
      double yn = (fft_y_real[i]-mean)/(2*sd);
