...
void generateInput(float *data_real, float *data_imag, int size) {
    for (int i = 0; i < size; i++) {
        data_real[i] = cos(2.0 * PI * i / size);
        data_imag[i] = sin(2.0 * PI * i / size);
    }
}

int compareArrays(float *arr1_real, float *arr1_imag, float *arr2_real, float *arr2_imag, int size, float tolerance) {
    for (int i = 0; i < size; i++) {
        float diff_real = arr1_real[i] - arr2_real[i];
        float diff_imag = 0; // We have optimized out the imaginary part
        if (fabs(diff_real) > tolerance || fabs(diff_imag) > tolerance) {
            return 0;  // Arrays differ
        }
    }
    return 1;  // Arrays are close enough
}

void printComplexArray(float *real, float *imag, int size) {
    for (int i = 0; i < size; i++) {
...
void testFFT() {
    ...
    float expected_output_real[size];
    float expected_output_imag[size];

    // Generate input data
    generateInput(input_real, input_imag, size);

    // Copy input for later comparison
    for (int i = 0; i < size; i++) {
        expected_output_real[i] = input_real[i];
        expected_output_imag[i] = input_imag[i];
    }

    // Call FFT
    fft(input_real, input_imag, size, 0);  // 0 for forward FFT

    // Print the input and output for verification
    printf("Input: ");
    printComplexArray(expected_output_real, expected_output_imag, size);

    printf("FFT Output: ");
    printComplexArray(input_real, input_imag, size);

    // Call Inverse FFT
    fft(input_real, input_imag, size, 1);  // 1 for inverse FFT

    // Print the inverse FFT output for verification
    printf("Inverse FFT Output: ");
    printComplexArray(input_real, input_imag, size);

    // Compare with the original data
    if (compareArrays(expected_output_real, expected_output_imag, input_real, input_imag, size, 1.0)) {
        printf("FFT and Inverse FFT are consistent. Data has been successfully reconstructed.\n");
    } else {
        printf("Warning: FFT and Inverse FFT may not be consistent. Data reconstruction failed.\n");
    }
}
